...
import { collection, getDocs, doc, updateDoc, query, where, addDoc, serverTimestamp, onSnapshot, Timestamp } from "firebase/firestore";
...
function ClientRemotePage() {
  ...
  const handleConfirmLost = useCallback(async (reason, details) => {
    ...
    try {
      await updateDoc(doc(db, "clients", lostTarget.id), { status: 'closed-lost', lostAtPhase: lostTarget.status, lostReason: reason, lostReasonDetails: details, updatedAt: serverTimestamp() });
      fetchClients();
    } 
  ...
  const handleConfirmEmployment = useCallback(async (employmentData) => {
    ...
    catch (error) { console.error("就職情報の更新エラー:", error); } 
    finally { 
  ...
  return (
    <Box sx={{ p: 2, pb: 12 }}>
      <Grid container spacing={2}>
        {clients.map(client => (
          // ▼▼▼ [ここを修正] 'item'プロパティを削除しました ▼▼▼
          <Grid key={client.id} xs={12} sm={6} md={4} lg={3}>
            <ClientCard
              client={client}
